...
function setup() {
  ...
  currentState = "RED_ROOM_2"; // Start with the curtain intro
  previousMouseY = mouseY;
  textFill = '#766d09';
  textOutline = '#5d9d38';
  red = color('#bb3131');
...
function draw() {
  ...
  if (currentState === "RED_ROOM_1" || currentState === "RED_ROOM_2" || currentState === "RED_ROOM_3") {
    ...
    if (redRoomMovementTimer > 2000) {
      ...
      if (currentState === "RED_ROOM_1" || currentState === "RED_ROOM_2" && glitchyPoemStartTime === 0) {
...
function changeState(newState) {
  ...
  // Reset poem trigger flag so each state can trigger poems
  poemTriggered = false;

  // Reset stillness and movement timers for red room states
  if (newState === "RED_ROOM_1" || newState === "RED_ROOM_2" || newState === "RED_ROOM_3" || newState === "FINAL_RED_ROOM") {
    redRoomStillnessTimer = 0;
    redRoomMovementTimer = 0;
  }
...
function drawDiner() {
  ...
  if (currentState === "DINER_1") {
    ...
    if (mouseX !== previousMouseX || mouseY !== previousMouseY) {
      ...
      // Delay erasing by 6 seconds after triggering (2 seconds after other effects)
      if (diner2Triggered && millis() - diner2EraseStartTime > 6000 && !diner2Erasing) {
        diner2Erasing = true;
...
function drawForest() {
  ...
  if (forestStillnessTimer > 7000) {
    ...
    textSize(24);
    textAlign(CENTER, CENTER);
    text("Your Laura disappeared.\nIt's just me now.", frameSizeWidth / 2, frameSizeHeight - 100);
    pop();
  }
